...
// <summary></summary>
// ***********************************************************************
using Zeroit.Framework.Components.Modernized.Objects.Interaction;
using System;
...
namespace Zeroit.Framework.Components.Modernized.Utils
{
    ...
    class FormUtils
    {
        ...
        {
            var minimum = f is Zeroit.Framework.Components.Modernized.Forms.ModernForm modernForm ? modernForm.MinimumSize : f.MinimumSize;
            var maximum = f is Zeroit.Framework.Components.Modernized.Forms.ModernForm form ? form.MaximumSize : f.MaximumSize;
                ...
                if (f.WindowState != FormWindowState.Maximized) {
                    ...
                    switch (resultEnum) {
                        case ResizeResult.Left:
                            if (curLoc.X <= Zeroit.Framework.Components.Modernized.Forms.ModernForm.SizingBorder && ((f.Width + changedSize) >= minimum.Width)) {
                                f.Left -= changedSize;
                        ...
                        case ResizeResult.Right:
                            if (curLoc.X >= minimum.Width - Zeroit.Framework.Components.Modernized.Forms.ModernForm.SizingBorder) {
                                f.Left = fLoc.X;
                        ...
                        case ResizeResult.Bottom:
                            if (curLoc.Y >= minimum.Height - Zeroit.Framework.Components.Modernized.Forms.ModernForm.SizingBorder) {
                                f.Top = fLoc.Y;
                                curLoc = f.PointToClient(Cursor.Position);
                        ...
                        case ResizeResult.BottomLeft:
                            if (curLoc.X <= Zeroit.Framework.Components.Modernized.Forms.ModernForm.SizingBorder && curLoc.Y >= minimum.Height - Zeroit.Framework.Components.Modernized.Forms.ModernForm.SizingBorder && ((f.Width + changedSize) >= minimum.Width)) {
        ...
        public static void StartFormDragFromTitlebar(System.Windows.Forms.Form f, Control c = null)
        {
            ...
            Action<Object, MouseEventArgs> mouseMove = (Object s, MouseEventArgs e) => {
                if (f.WindowState == FormWindowState.Maximized) {
                    var beforeW = (f is Zeroit.Framework.Components.Modernized.Forms.ModernForm ? (((Zeroit.Framework.Components.Modernized.Forms.ModernForm)f).TextBarRectangle.Width) : f.Width);
                    f.WindowState = FormWindowState.Normal;
                    var afterW = (f is Zeroit.Framework.Components.Modernized.Forms.ModernForm ? (((Zeroit.Framework.Components.Modernized.Forms.ModernForm)f).TextBarRectangle.Width) : f.Width);
